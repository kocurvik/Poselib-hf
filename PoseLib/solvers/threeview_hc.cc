...
struct track_settings {
    track_settings()
        : init_dt_(0.05),                                          // m2 tStep, t_step, raw interface code initDt
          ...
          end_zone_factor_(0.05), epsilon_(4e-2),                  // m2 CorrectorTolerance
          ...
          max_corr_steps_(
              9), // m2 maxCorrSteps (track.m2 param of rawSetParametersPT corresp to max_corr_steps in NAG.cpp)
          num_successes_before_increase_(4), // m2 numberSuccessesBeforeIncrease
          corr_thresh_(0.0001), anch_num_(134) {}
...
template <unsigned N, typename F> struct minus_array { // Speed critical -----------------------------------------
    static inline void multiply_scalar_to_self(F *__restrict__ a, F b) {
        for (unsigned i = 0; i < N; ++i, ++a)
            *a = *a * b;
    }

    static inline void negate_self(F *__restrict__ a) {
        for (unsigned i = 0; i < N; ++i, ++a)
            *a = -*a;
    }

    static inline void multiply_self(F *__restrict__ a, const F *__restrict__ b) {
        for (unsigned int i = 0; i < N; ++i, ++a, ++b)
            *a *= *b;
    }

    static inline void add_to_self(F *__restrict__ a, const F *__restrict__ b) {
        for (unsigned int i = 0; i < N; ++i, ++a, ++b)
            *a += *b;
    }

    static inline void add_scalar_to_self(F *__restrict__ a, F b) {
        for (unsigned int i = 0; i < N; ++i, ++a)
            *a += b;
    }

    static inline void copy(const F *__restrict__ a, F *__restrict__ b) { memcpy(b, a, N * sizeof(double)); }

    static inline F norm2(const F *__restrict__ a) {
        F val = 0;
...
int track(const struct track_settings s, const double s_sols[9], const double params[40], double solution[9],
          int *num_st) {
    ...
    Eigen::Map<const Eigen::Matrix<double, nve, nve>, Eigen::Aligned> AA((double *)Hxt, nve,
    ...
    *dt = t_step;
    ...
    while (status == 1 && 1 - *t0 > the_smallest_number) {
        ...
        evaluate_Hxt(xt, params, Hxt); // Outputs Hxt
        {
            ...
            const double c40 =
                -AA(8, 0) - AA(8, 1) * e1 + AA(8, 8) * AA(6, 0) / AA(6, 8) + AA(8, 9) * AA(7, 1) * e1 / AA(7, 9);
            const double d4 =
                bb(8) - AA(8, 1) * f1 - (AA(8, 8) / AA(6, 8)) * bb(6) - (AA(8, 9) / AA(7, 9)) * (bb(7) - AA(7, 1) * f1);

            const double c9 = AA(9, 2) * e2 + AA(9, 7) * c40 / c47;
            const double d9 = bb(9) - AA(9, 2) * f2 - AA(9, 7) * d4 / c47;

            const double c010 = AA(10, 0) + AA(10, 2) * e2 - AA(10, 8) * AA(6, 0) / AA(6, 8) -
                                AA(10, 8) * AA(6, 7) * c40 / (AA(6, 8) * c47);
            const double d010 = bb(10) - AA(10, 2) * f2 - (AA(10, 8) / AA(6, 8)) * bb(6) +
                                (AA(10, 8) * AA(6, 7) * d4) / (AA(6, 8) * c47);

            const double c011 = AA(11, 1) * e1 + AA(11, 2) * e2 - AA(11, 9) * AA(7, 1) * e1 / AA(7, 9) -
                                AA(11, 9) * AA(7, 7) * c40 / (AA(7, 9) * c47);
            const double d011 = bb(11) - AA(11, 1) * f1 - AA(11, 2) * f2 - AA(11, 9) * bb(7) / AA(7, 9) +
                                AA(11, 9) * AA(7, 1) * f1 / AA(7, 9) + (AA(11, 9) * AA(7, 7) * d4) / (AA(7, 9) * c47);
            ...
            const double g0 = c011 - AA(11, 10) * e010 / e1010 - AA(11, 11) * c9 / AA(9, 11) +
                              AA(11, 11) * AA(9, 10) * e010 / (AA(9, 11) * e1010);
            const double h0 = d011 - AA(11, 10) * f10 / e1010 - AA(11, 11) * d9 / AA(9, 11) +
                              AA(11, 11) * AA(9, 10) * f10 / (AA(9, 11) * e1010);
        ...
        v::multiply_scalar_to_self(
            dxA, 1. / 6.); //(1/6) * (dt*dx1 + 2*dt*dx2 + 2*dt*dx3 + dt*dx4 in dxB) in dxA = dx (the RK prediction)
...
void poselib::ThreeViewRelativePoseHCEstimator::generate_models(std::vector<ThreeViewCameraPose> *models) {
    ...
    for (int i = 0; i < layers; ++i) {
        ...
        const Eigen::Map<const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>, Eigen::Aligned>
            weights =
                Eigen::Map<const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>, Eigen::Aligned>(
                    ws_, a_[i], b_[i]);
    ...
    if (p == -1)
        return; // tsattler: I assume this means failure. Not 100% sure.
    // continue;
...
double poselib::ThreeViewRelativePoseHCEstimator::score_model(const ThreeViewCameraPose &three_view_pose,
    ...
    double score12 = compute_sampson_msac_score(three_view_pose.pose12, xx1, xx2,
                                                opt.max_epipolar_error * opt.max_epipolar_error, &inlier_count12);
    double score13 = compute_sampson_msac_score(three_view_pose.pose13, xx1, xx3,
                                                opt.max_epipolar_error * opt.max_epipolar_error, &inlier_count13);
    double score23 = compute_sampson_msac_score(three_view_pose.pose23(), xx2, xx3,
    ...
    *inlier_count =
        get_inliers(three_view_pose, xx1, xx2, xx3, opt.max_epipolar_error * opt.max_epipolar_error, &inliers);
...
void ThreeViewRelativePoseHCEstimator::extract_pose12(double params[24], double solution[12], Eigen::Matrix3d &R,
...
void ThreeViewRelativePoseHCEstimator::extract_pose13(double params[24], double solution[12], Eigen::Matrix3d &R,
                                                      Eigen::Vector3d &t) const {
    const Eigen::Vector3d x1(solution[0] * params[0], solution[0] * params[4], solution[0]);
    const Eigen::Vector3d x2(solution[1] * params[1], solution[1] * params[5], solution[1]);
    const Eigen::Vector3d x3(solution[2] * params[2], solution[2] * params[6], solution[2]);
...
void ThreeViewRelativePoseHCEstimator::normalize(std::vector<Eigen::Vector3d> &P, std::vector<Eigen::Vector3d> &Q,
                                                 std::vector<Eigen::Vector3d> &R, std::vector<Eigen::Vector2d> &P1,
                                                 std::vector<Eigen::Vector2d> &Q1, std::vector<Eigen::Vector2d> &R1,
                                                 Eigen::Matrix3d &CP1, Eigen::Matrix3d &CQ1, Eigen::Matrix3d &CR1,
                                                 int *perm, int *perm4) const {
